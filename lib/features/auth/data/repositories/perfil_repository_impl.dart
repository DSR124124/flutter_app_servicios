...
class PerfilRepositoryImpl implements PerfilRepository {
  ...
  Future<PerfilInfo> obtenerPerfil() async {
    ...
    // Combinar datos del perfil con datos del usuario del login
    // El login response tiene más información (email, nombreCompleto, etc.)
    // que no está en el perfil del backend de servicios
    final combinedData = Map<String, dynamic>.from(data);
    
    // Completar campos faltantes con datos del login si están disponibles
    if (user != null) {
